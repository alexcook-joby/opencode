...
User display names are optional for attached clients. A name is only sent when you pass `--name`, with no fallback from `USER`/`USERNAME` and no interactive name prompt.
...
| `--name`     |       | Optional display name to send with messages |
